...
class ImageEncoderViT3D(nn.Module):
    ...
    def forward(self, x: torch.Tensor, hidden_out=False) -> torch.Tensor:
        ...
        if hidden_out:
            ...
            for blk in self.blocks:
                x = blk(x)
                hidden_states_out.append(x.permute(0, 4, 1, 2, 3).contiguous())
            # x = [1,16,16,16,768]
...
# This class and its supporting functions below lightly adapted from the ViTDet backbone available at: https://github.com/facebookresearch/detectron2/blob/main/detectron2/modeling/backbone/vit.py # noqa
class ImageEncoderViT3D_small(nn.Module):
    def __init__(
        self,
        img_size: int = 256,
        patch_size: int = 16,
    ...
    def forward(self, x: torch.Tensor, hidden_out=False) -> torch.Tensor:
        # input_size = [1,1,256,256,256]
        # import IPython; IPython.embed()
        x = self.patch_embed(x) # torch.Size([2, 8, 8, 8, 768])
        # x = [1,16,16,16,768]
        # import pdb; pdb.set_trace()
        if self.pos_embed is not None:
            x = x + self.pos_embed # x size: torch.Size([2, 8, 8, 8, 768]), pos_embed size: torch.Size([1, 8, 8, 8, 768])
        ...
        if hidden_out:
            ...
            for blk in self.blocks:
                x = blk(x)
            # x = [1,16,16,16,768] torch.Size([2, 8, 8, 8, 768])
            x = self.neck(x.permute(0, 4, 1, 2, 3)) # torch.Size([2, 384, 8, 8, 8])
