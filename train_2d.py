...
parser.add_argument('--dataset', type=str,
                    default='OASIS2D', help='dataset name')
...
parser.add_argument('--data_dir', type=str, help='Data source')
...
def get_OASIS_2DLargestCC(segmentation):
    batch_list = []
    ...
    for i in range(0, N):
        ...
        n_batch = class_list[0] + class_list[1] + class_list[2] + class_list[3]
        batch_list.append(n_batch)

    return torch.Tensor(batch_list).cuda()
    

def get_OASIS_masks(output, nms=0):
    probs = F.softmax(output, dim=1)
    _, probs = torch.max(probs, dim=1)
    if nms == 1:
        probs = get_OASIS_2DLargestCC(probs)      
    return probs
...
def pretrain(args, snapshot_path):
    ...
    db_train = BaseDataSets(base_dir=args.data_dir, split="train", num=args.labeled_num, fold_num=args.fold_num, transform=transforms.Compose([
...
def ssl_train(args, snapshot_path):
    ...
    db_train = BaseDataSets(base_dir=args.data_dir, split="train", num=None, fold_num=args.fold_num, transform=transforms.Compose([
        RandomGenerator(args.patch_size), 
    ]))
    db_val = BaseDataSets(base_dir=args.data_dir, split="val", fold_num=args.fold_num)
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            with torch.no_grad():
                ...
                plab_a = get_OASIS_masks(unoutput_a, nms=1)
                plab_b = get_OASIS_masks(unoutput_b, nms=1)
