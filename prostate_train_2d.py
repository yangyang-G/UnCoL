...
from datasets.prostate import Prostate, RandomGenerator as ProstateRandomGenerator
...
parser.add_argument('--dataset', type=str,
                    default='Prostate', help='dataset name')
...
parser.add_argument('--data_dir', type=str, default='../../data/Prostate', help='Data source')
...
def pretrain(args, snapshot_path):
    ...
    db_train = Prostate(base_dir=args.data_dir, split="train", num=args.labeled_num, transform=transforms.Compose([
        ProstateRandomGenerator(args.patch_size),
    ]))
    db_val = Prostate(base_dir=args.data_dir, split="val")
...
def ssl_train(args, snapshot_path):
    ...
    def worker_init_fn(worker_id):
        random.seed(args.seed + worker_id)
    db_train = Prostate(base_dir=args.data_dir, split="train", transform=transforms.Compose([
        ProstateRandomGenerator(args.patch_size),
    ]))
    db_val = Prostate(base_dir=args.data_dir, split="val")
