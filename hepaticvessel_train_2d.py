...
from datasets.hepaticvessel import HepaticVessel, RandomGenerator as HepaticVesselRandomGenerator
...
from utils.train_utils import update_ema_variables
from utils.BCP_utils import generate_mask, masked_mix_loss


parser = argparse.ArgumentParser()
parser.add_argument("--method", type=str, default="UnCoL", help="method_name")
parser.add_argument("--dataset", type=str, default="HepaticVessel", help="dataset name")
parser.add_argument("--mode", type=str, default="PRETRAIN", help="mode type")
parser.add_argument("--model", type=str, default="unet", help="model_name")
parser.add_argument("--num_classes", type=int, default=3, help="output channel of network")
parser.add_argument("--fold_num", type=int, default=0, help="fold num of dataset")
parser.add_argument("--data_dir", type=str, default="../../data/HepaticVessel", help="Data source")
...
def get_2DLargestCC(segmentation, num_classes):
    ...
    for i in range(segmentation.shape[0]):
        ...
        batch_list.append(sum(class_list))
    return torch.Tensor(batch_list).cuda()


def get_masks(output, nms=0, num_classes=3):
...
def pretrain(args, snapshot_path):
    ...
    db_train = HepaticVessel(
        base_dir=args.data_dir,
        split="train",
        num=args.labeled_num,
        transform=transforms.Compose([HepaticVesselRandomGenerator(args.patch_size)]),
    )
    db_val = HepaticVessel(base_dir=args.data_dir, split="val")
    ...
    for _ in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            with torch.no_grad():
                img_mask, _ = generate_mask(img_a, args.mask_ratio)
            ...
            with torch.no_grad():
                hidden_states_out_sam, posterior_features, _, _ = get_medsam_features(
                    volume_batch, label_batch, medsam_model, num_classes=args.num_classes
                )

            outputs, loss_kd, loss_infomax = model(
                volume_batch, hidden_states_out_sam=hidden_states_out_sam, sam_post_embed=posterior_features
            )
            ...
            logging.info(
                "iteration {} : loss : {:.4f}, loss_sup: {:.4f}, loss_kd: {:.4f}, loss_infomax: {:.4f}".format(
                    iter_num, loss.item(), loss_sup.item(), loss_kd.item(), loss_infomax.item()
                )
            )
            ...
            if iter_num > 0 and iter_num % 200 == 0:
                ...
                for _, sampled_batch in tqdm(enumerate(valloader)):
                ...
                if total_metric[:, 0].mean() > best_performance:
                    ...
                    torch.save(ema_model.state_dict(), os.path.join(snapshot_path, "best_ema_model.pth"))

                for i in range(num_classes - 1):
                    writer.add_scalar("val/class{}_val_dice_score".format(i + 1), total_metric[i, 0], iter_num)
                    writer.add_scalar("val/class{}_val_hd95".format(i + 1), total_metric[i, 2], iter_num)
                    logging.info(
                        "Class {}, iteration {} : dice_score : {:.2f}% jaccard : {:.2f}%  hd95 : {:.2f} assd : {:.2f}".format(
                            i + 1,
                            iter_num,
                            total_metric[i, 0] * 100,
                            total_metric[i, 1] * 100,
                            total_metric[i, 2],
                            total_metric[i, 3],
                        )
                    )
                logging.info(
                    "{:.2f}\t{:.2f}\t{:.2f}\t{:.2f}".format(
                        total_metric[:, 0].mean() * 100,
                        total_metric[:, 1].mean() * 100,
                        total_metric[:, 2].mean(),
                        total_metric[:, 3].mean(),
                    )
                )
...
def fuse_pseudo_label_with_mask_fast(
    p_foundation, p_teacher, u_foundation, u_teacher, uncertainty_threshold=1.0
):
    ...
    pseudo_fused = both_trust * fused + only_teacher * p_teacher + only_foundation * p_foundation
    ...
    for _ in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            with torch.no_grad():
                ...
                _, _, _, p_foundation_a = get_medsam_features(
                    unimg_a, plab_a, medsam_model, num_classes=args.num_classes
                )
                _, _, _, p_foundation_b = get_medsam_features(
            ...
            newplab_a, plab_mask_a = fuse_pseudo_label_with_mask_fast(
                p_foundation_a, p_teacher_a, u_foundation_a, u_teacher_a, uncertainty_threshold=threshold
            )
            newplab_b, plab_mask_b = fuse_pseudo_label_with_mask_fast(
                p_foundation_b, p_teacher_b, u_foundation_b, u_teacher_b, uncertainty_threshold=threshold
            )
            loss_l = masked_mix_loss(
                outputs_l,
                lab_a,
                newplab_a,
                loss_mask,
            ...
            logging.info(
                "iteration {} : loss : {:.4f}, lr : {:.4f}, loss_infomax : {:.4f}, loss_l : {:.4f},  loss_u : {:.4f}".format(
                    iter_num, loss.item(), lr_, loss_infomax.item(), loss_l.item(), loss_u.item()
                )
            )
...
if __name__ == "__main__":
    if not args.checkpoint_sam:
        raise ValueError("Please provide --checkpoint_sam.")
    if args.mode != "PRETRAIN" and not args.checkpoint_unet:
        raise ValueError("Please provide --checkpoint_unet for SSL mode.")
    ...
    snapshot_path = os.path.join(
        args.save_path, "{}/fold-{}/lab-{}/{}/{}".format(args.dataset, args.fold_num, args.labeled_num, args.method, args.mode)
    )
    if not os.path.exists(snapshot_path):
        os.makedirs(snapshot_path)
    if os.path.exists(snapshot_path + "/codes"):
        shutil.rmtree(snapshot_path + "/codes")
    shutil.copytree(".", snapshot_path + "/codes", shutil.ignore_patterns([".git", "__pycache__"]))
