...
def fast_context_mask_tensorized(img, lab, mask_ratio=2/3, margin=4, min_size=8):
    """
    Fully tensorized, GPU-based fast version for generating context masks.
    No CPU ↔ GPU transfer. Vectorized mask generation.

    Args:
        img: (B, C, X, Y, Z)
        lab: (B, 1, X, Y, Z)
        mask_ratio: float in (0,1), controls masking proportion if no foreground
        margin: int, extra voxels to include around foreground
        min_size: int, minimal cube size to mask even if no foreground

    Returns:
        masks:      [B, X, Y, Z] Tensor of context masks
        loss_masks: [B, X, Y, Z] Loss masks
    """
    B, _, X, Y, Z = img.shape
    device = img.device

    masks = torch.ones((B, X, Y, Z), device=device)
    loss_masks = torch.ones((B, X, Y, Z), device=device)

    # Identify foreground regions
    fg = (lab > 0)  # [B, X, Y, Z]

    # Identify which samples have foreground regions
    has_fg = fg.view(B, -1).any(dim=1)  # [B]
    # Generate bounding box for foreground region
    for b in range(B):
        if has_fg[b]:
            coords = torch.nonzero(fg[b], as_tuple=False)  # [N, 3]
            min_coord = torch.clamp(coords.min(dim=0)[0] - margin, min=0)
            max_coord = torch.clamp(coords.max(dim=0)[0] + margin + 1, max=torch.tensor([X, Y, Z], device=device))
            x0, y0, z0 = min_coord.tolist()
            x1, y1, z1 = max_coord.tolist()
        else:
            # No foreground: generate random cube
            rx = max(int(X * mask_ratio), min_size)
            ry = max(int(Y * mask_ratio), min_size)
            rz = max(int(Z * mask_ratio), min_size)
            x0 = torch.randint(0, X - rx + 1, (1,)).item()
            y0 = torch.randint(0, Y - ry + 1, (1,)).item()
            z0 = torch.randint(0, Z - rz + 1, (1,)).item()
            x1, y1, z1 = x0 + rx, y0 + ry, z0 + rz
        masks[b, x0:x1, y0:y1, z0:z1] = 0  # masked region
        loss_masks[b, x0:x1, y0:y1, z0:z1] = 0  # loss mask
    return masks.long(), loss_masks.long()
...
def context_mask(img, mask_ratio):
    ...
    w = np.random.randint(0, 128 - patch_pixel_x)
    h = np.random.randint(0, 128 - patch_pixel_y)
    z = np.random.randint(0, 128 - patch_pixel_z)
...
def generate_mask(img, mask_ratio):
...
def mix_loss(net3_output, img_l, patch_l, mask, l_weight=1.0, u_weight=0.5, unlab=False, num_classes=3):
...
def masked_mix_loss(net3_output, img_l, patch_l, mask, threshold_mask, l_weight=1.0, u_weight=0.5, unlab=False, num_classes=3):
    DICE = mask_DiceLoss(nclass=num_classes)
    img_l, patch_l = img_l.type(torch.int64), patch_l.type(torch.int64)
    image_weight, patch_weight = l_weight, u_weight
    if unlab:
        ...
        patch_mask = (1 - mask)*threshold_mask
    dice_loss = DICE(net3_output, img_l, mask) * image_weight 
    dice_loss += DICE(net3_output, patch_l, patch_mask) * patch_weight
    loss_ce = image_weight * (CE(net3_output, img_l) * mask).sum() / (mask.sum() + 1e-16) 
    loss_ce += patch_weight * (CE(net3_output, patch_l) * patch_mask).sum() / (patch_mask.sum() + 1e-16)
