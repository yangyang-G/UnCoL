...
parser.add_argument('-o', '--seg_path', type=str, default="/amax/data/luwenjing/P4_UKD-SAM/results/SAM2D")
...
data_dir = '/amax/data/luwenjing/P1_UPCoL/Datasets/OASIS_256Slice' 
name_list = '/amax/data/luwenjing/P1_UPCoL/Datasets/OASIS_256Slice/datalist/fold_0/test.txt'

with open(name_list, 'r') as f:
    data_list = f.readlines()

data_list = [item.replace('\n', '').split(",")[0] for item in data_list]
data_list = [os.path.join(data_dir, item + ".h5") for item in data_list]

print('find {} files'.format(len(data_list)))
image_size = 1024
bbox_shift = 20
...
for image_name in data_list:
    ...
    gt_file_name = os.path.basename(image_name)
    task_folder = gt_file_name[:-3]
    ...
    print("{}".format(join(seg_path, task_folder)))
    ...
    for label_id in label_ids:
        ...
        print("class {}, {:.2f}\t{:.2f}\t{:.2f}\t{:.2f}".format(label_id, score[0]*100, score[1]*100, score[2], score[3]))
...
print("class {}, {:.2f}\t{:.2f}\t{:.2f}\t{:.2f}".format(i+1,
            total_scores[:, i, 0].mean()*100, total_scores[:, i, 1].mean()*100, 
            total_scores[:, i, 2].mean(), total_scores[:, i, 2].mean()))
