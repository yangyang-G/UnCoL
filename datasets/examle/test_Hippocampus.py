...
parser.add_argument('--root_path', type=str,
                    default='../../data/Hippocampus', help='Name of Experiment')
parser.add_argument('--exp', type=str,
                    default='train_Hippocampus_Cross_Teaching', help='experiment_name')
...
def Inference_model1(FLAGS):
    ...
    snapshot_path = "../model/Cross_Teaching/Hippocampus_{}_{}_labeled".format(FLAGS.exp, FLAGS.labeled_num)
    test_save_path = "../model/Cross_Teaching/Hippocampus_{}_{}_labeled/{}_predictions_model/".format(FLAGS.exp, FLAGS.labeled_num, FLAGS.model_1)
...
def Inference_model2(FLAGS):
    ...
    with open(FLAGS.root_path + '/test.list', 'r') as f:image_list = f.readlines()
    image_list = sorted([item.replace('\n', '').split(".")[0]for item in image_list])
    snapshot_path = "../model/Cross_Teaching/Prostate_{}_{}_labeled".format(FLAGS.exp, FLAGS.labeled_num)
    test_save_path = "../model/Cross_Teaching/Prostate_{}_{}_labeled/{}_predictions_model/".format(FLAGS.exp, FLAGS.labeled_num, FLAGS.model_2)
    if not os.path.exists(test_save_path):
        os.makedirs(test_save_path)
