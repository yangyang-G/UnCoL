...
parser.add_argument('--root_path', type=str,
                    default='../../data/HepaticVessel', help='Name of Experiment')
parser.add_argument('--exp', type=str,
                    default='train_HepaticVessel_Cross_Teaching', help='experiment_name')
...
def test_single_volume(case, net, test_save_path, FLAGS):
    ...
    for ind in range(image.shape[0]):
        ...
        slice = zoom(slice, (FLAGS.image_size[0] / x, FLAGS.image_size[1] / y), order=0)
        ...
        with torch.no_grad():
            ...
            pred = zoom(out, (x / FLAGS.image_size[0], y / FLAGS.image_size[1]), order=0)
            prediction[ind] = pred
...
def Inference_model1(FLAGS):
    ...
    snapshot_path = "../model/Cross_Teaching/HepaticVessel_{}_{}_labeled".format(FLAGS.exp, FLAGS.labeled_num)
    test_save_path = "../model/Cross_Teaching/HepaticVessel_{}_{}_labeled/{}_predictions/".format(FLAGS.exp, FLAGS.labeled_num, FLAGS.model_1)
    if not os.path.exists(test_save_path):
        os.makedirs(test_save_path)
    net = net_factory(net_type=FLAGS.model_1, in_chns=1,class_num=FLAGS.num_classes)
    save_model_path = os.path.join(snapshot_path, '{}_best_model.pth'.format(FLAGS.model_1))
...
def Inference_model2(FLAGS):
    ...
    image_list = sorted([item.replace('\n', '').split(".")[0]for item in image_list])
    snapshot_path = "../model/Cross_Teaching/HepaticVessel_{}_{}_labeled".format(FLAGS.exp, FLAGS.labeled_num)
    test_save_path = "../model/Cross_Teaching/HepaticVessel_{}_{}_labeled/{}_predictions/".format(FLAGS.exp, FLAGS.labeled_num, FLAGS.model_2)
    if not os.path.exists(test_save_path):
        os.makedirs(test_save_path)
...
if __name__ == '__main__':
    ...
    # 测试模型1
    metric1, test_save_path1 = Inference_model1(FLAGS)
    for cls in range(1, FLAGS.num_classes):
        print(f"Model1 Class {cls:2d} => Dice: {metric1[cls - 1, 0]:.4f}, JC: {metric1[cls - 1, 1]:.4f}, HD95: {metric1[cls - 1, 2]:.2f}, ASD: {metric1[cls - 1, 3]:.2f}")

    mean_avg1 = np.mean(metric1, axis=0)
    print(f"\nModel1 Overall Average => Dice: {mean_avg1[0]:.4f}, JC: {mean_avg1[1]:.4f}, HD95: {mean_avg1[2]:.2f}, ASD: {mean_avg1[3]:.2f}")

    with open(test_save_path1 + '../model1_performance.txt', 'w') as f:
        f.writelines("Per-class metrics:\n")
        for cls in range(1, FLAGS.num_classes):
            f.writelines(f"Class {cls:2d} => Dice: {metric1[cls - 1, 0]:.4f}, JC: {metric1[cls - 1, 1]:.4f}, HD95: {metric1[cls - 1, 2]:.2f}, ASD: {metric1[cls - 1, 3]:.2f}\n")
        f.writelines(f"\nOverall Average => Dice: {mean_avg1[0]:.4f}, JC: {mean_avg1[1]:.4f}, HD95: {mean_avg1[2]:.2f}, ASD: {mean_avg1[3]:.2f}\n")
    
    # 测试模型2
    metric2, test_save_path2 = Inference_model2(FLAGS)
    for cls in range(1, FLAGS.num_classes):
        print(f"Model2 Class {cls:2d} => Dice: {metric2[cls - 1, 0]:.4f}, JC: {metric2[cls - 1, 1]:.4f}, HD95: {metric2[cls - 1, 2]:.2f}, ASD: {metric2[cls - 1, 3]:.2f}")

    mean_avg2 = np.mean(metric2, axis=0)
    print(f"\nModel2 Overall Average => Dice: {mean_avg2[0]:.4f}, JC: {mean_avg2[1]:.4f}, HD95: {mean_avg2[2]:.2f}, ASD: {mean_avg2[3]:.2f}")

    with open(test_save_path2 + '../model2_performance.txt', 'w') as f:
        f.writelines("Per-class metrics:\n")
        for cls in range(1, FLAGS.num_classes):
            f.writelines(f"Class {cls:2d} => Dice: {metric2[cls - 1, 0]:.4f}, JC: {metric2[cls - 1, 1]:.4f}, HD95: {metric2[cls - 1, 2]:.2f}, ASD: {metric2[cls - 1, 3]:.2f}\n")
        f.writelines(f"\nOverall Average => Dice: {mean_avg2[0]:.4f}, JC: {mean_avg2[1]:.4f}, HD95: {mean_avg2[2]:.2f}, ASD: {mean_avg2[3]:.2f}\n")